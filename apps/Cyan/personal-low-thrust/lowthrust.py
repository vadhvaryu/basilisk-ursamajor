...
# ---------------- SCENARIO ----------------
def scenarioLowThrust(show_plots=True):
    ...
    sc.hub.mHub = m0
    sc.hub.r_CN_NInit = [r_LEO, 0, 0]
    sc.hub.v_CN_NInit = [0, v_LEO, 0]
    rk4 = svIntegrators.svIntegratorRK4(sc)
    sc.setIntegrator(rk4)
    ...
    sc.addDynamicEffector(thrusterEffector)
    scSim.AddModelToTask(simTaskName, thrusterEffector)
    scSim.AddModelToTask(simTaskName, thr_device)

    # Thrust command
    thrustPayload = messaging.DoubleVecPayload()
    thrustPayload.vec = [0.0]
    thrustMsg = messaging.DoubleVecMsg().write(thrustPayload)
    thrusterEffector.thrusterCmdInMsg.subscribeTo(thrustMsg)

    # Logging
    stateLog = sc.scStateOutMsg.recorder()
    ...
    while step < max_steps:
        ...
        if thrusting and r_mag >= r_GEO:
            ...
            thrusterEffector.thrusterCmdInMsg.write(
                messaging.DoubleVecMsg().write(messaging.DoubleVecPayload())
            )
